...
class Helpers
{
	...
	public static function render(
		...
		$path = self::joinPaths([self::getProjectPaths($pathName), $prefix, "{$name}.php"]);
		...
		if ($useComponentDefaults) {
			$manifest = self::getComponent($name);
		}
	...
	public static function getManifestByDir(string $path): array
	{
		...
		$root = self::getProjectPaths('srcDestination');
		...
		switch ($newPath[1]) {
			...
			case 'components':
				return self::getComponent(\end($newPath));
			case 'custom':
				return self::getBlock(\end($newPath));
			default:
