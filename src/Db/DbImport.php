...
function db_import( string $project_root_path, array $args = [], string $setup_file = 'setup.json' ) {
  ...
  if ( file_exists( $export_folder_name ) ) {
    \WP_CLI::log( shell_exec( "rm -rf {$export_folder_name}" ) ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.system_calls_shell_exec
  ...
  \WP_CLI::log( shell_exec( "tar zxf {$export_file_name} -C {$export_folder_name}" ) ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.system_calls_shell_exec
